...
// url absoluto da página "sobre" — reforça para motores generativos (AIO/GEO)
// que esta página é a fonte de história/factos do restaurante, distinta da
// home (que já tem Restaurant+WebSite globais no layout).
export function buildAboutPageSchema({ locale, title, description }) {
  return {
    "@context": "https://schema.org",
    "@type": "AboutPage",
    name: title,
    description,
    url: `${BASE_URL}${locale === "pt" ? "" : "/" + locale}/pizzarte`,
    about: {
      "@type": "Restaurant",
      name: SITE_NAME,
    },
...
// images: [{ src: string (relativo a /images), alt: string }]
export function buildImageGallerySchema(images, { locale, name, description }) {
  return {
    "@context": "https://schema.org",
    "@type": "ImageGallery",
    name,
    description,
    url: `${BASE_URL}${locale === "pt" ? "" : "/" + locale}/galeria`,
    associatedMedia: images.map((image) => ({
      "@type": "ImageObject",
      contentUrl: `${BASE_URL}/images/${image.src}`,
      caption: image.alt,
    })),
  };
}
